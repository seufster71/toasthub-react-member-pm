	...
	switch(action.type) {
		case 'PM_TASK_INIT': {
			if (action.responseJson != null && action.responseJson.params != null) {
				return Object.assign({}, state, {
					...
					orderCriteria: [{'orderColumn':'PM_TASK_TABLE_NAME','orderDir':'ASC'}],
    				searchCriteria: [{'searchValue':'','searchColumn':'PM_TASK_TABLE_NAME'}],
    				paginationSegment: 1,
					selected: null,
					view: "MAIN",
					pageName:"PMTASK",
		...
		case 'PM_TASK_ITEM': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				...
				for (let i = 0; i < prefForms.PM_TASK_FORM.length; i++) {
					if (prefForms.PM_TASK_FORM[i].group === "FORM1") {
						let classModel = JSON.parse(prefForms.PM_TASK_FORM[i].classModel);
						if (action.responseJson.params.item != null && action.responseJson.params.item[classModel.field] != null) {
							inputFields[prefForms.PM_TASK_FORM[i].name] = action.responseJson.params.item[classModel.field];
						} else {
							let result = "";
							if (prefForms.PM_TASK_FORM[i].value != null && prefForms.PM_TASK_FORM[i].value != ""){
								let formValue = JSON.parse(prefForms.PM_TASK_FORM[i].value);
		...
		case 'PM_TASK_INPUT_CHANGE': {
			return reducerUtils.updateInputChange(state,action);
		}
		case 'PM_TASK_CLEAR_FIELD': {
			return reducerUtils.updateClearField(state,action);
		}
		case 'PM_TASK_LISTLIMIT': {
			return reducerUtils.updateListLimit(state,action);
		}
		case 'PM_TASK_SEARCH': { 
			return reducerUtils.updateSearch(state,action);
		}
		case 'PM_TASK_SEARCH_CHANGE': { 
		...
		case 'PM_TASK_SET_ERRORS': {
			return Object.assign({}, state, {
				errors: action.errors
			});
		}
		case 'PM_TASK_CLOSE_DELETE_MODAL': {
			return Object.assign({}, state, {
				isDeleteModalOpen: false
			});
		}
		case 'PM_TASK_OPEN_DELETE_MODAL': {
			return Object.assign({}, state, {
