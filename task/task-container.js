...
function PMTaskContainer({location,navigate}) {
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"PM_TASK_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code, item) => {
		...
		switch(code) {
			case 'COMMENT': {
				newPath = newPath + "/pm-comment";
				navigate(newPath,{state:{parent:item,parentType:"TASK"}});
				break;
			}
		}
	}
...
export default PMTaskContainer;
