...
export function init({parent,parentType}) {
	return function(dispatch) {
		...
		if (parent != null) {
			...
			dispatch({type:"PM_MEMBER_ADD_PARENT", parent, parentType});
		} else {
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_MEMBER_INIT", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_MEMBER_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function searchChange({field,value}) {
	return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"PM_MEMBER_SEARCH_CHANGE",params});
	 };
}
...
export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"PM_MEMBER_CANCEL"});
		dispatch(list({state}));
	 };
}
