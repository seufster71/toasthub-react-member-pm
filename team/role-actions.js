...
// thunks
export function init({parent,parentType,teamId}) {
	return function(dispatch) {
		...
		if (parentType == "TEAM") {
			requestParams.prefTextKeys = new Array("PM_TEAM_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("PM_TEAM_ROLE_PAGE");
		} else if (parentType == "MEMBER") {
			requestParams.prefTextKeys = new Array("PM_MEMBER_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("PM_MEMBER_ROLE_PAGE");
			requestParams.teamId = teamId;
		}
		
		if (parent != null) {
			requestParams.parentId = parent.id;
			dispatch({type:"PM_ROLE_ADD_PARENT", parent, parentType});
		} else {
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_ROLE_INIT", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		if (state.parentType == "TEAM") {
			requestParams.prefTextKeys = new Array("PM_TEAM_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("PM_TEAM_ROLE_PAGE");
		} else if (state.parentType == "MEMBER") {
			requestParams.prefTextKeys = new Array("PM_MEMBER_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("PM_MEMBER_ROLE_PAGE");
		}
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_ROLE_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function modifyItem({id, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "PM_ROLE_SVC";
	    requestParams.prefFormKeys = new Array("PM_TEAM_ROLE_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
...
export function modifyTeamRole({role, appPrefs}) {
	return function(dispatch) {
	    let requestParams = {};
	    requestParams.action = "TEAM_ROLE_ITEM";
	    requestParams.service = "PM_ROLE_SVC";
	    requestParams.prefFormKeys = new Array("PM_TEAM_ROLE_FORM");
	    if (role != null) {
	    	requestParams.itemId = role.id;
	    }
	    let params = {};
	    params.requestParams = requestParams;
	    params.URI = '/api/member/callService';
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'PM_ROLE_TEAM_ROLE',responseJson, appPrefs, role});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function clearItem() {
	return function(dispatch) {
		dispatch({ type:"PM_ROLE_CLEAR_ITEM"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"PM_ROLE_CLEAR_FIELD",params});
	};
}

export function setStatus({successes,errors}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_ROLE_SET_STATUS",successes,errors});
	 };
}
