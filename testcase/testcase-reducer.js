...
export default function testCaseReducer(state = {}, action) {
	...
	switch(action.type) {
		...
		case 'PM_TESTCASE_ITEM': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				// load inputFields
				let inputFields = {};
				let prefForms = reducerUtils.getPrefForms(action);
				for (let i = 0; i < prefForms.PM_TESTCASE_FORM.length; i++) {
					if (prefForms.PM_TESTCASE_FORM[i].group === "FORM1") {
						let classModel = JSON.parse(prefForms.PM_TESTCASE_FORM[i].classModel);
						if (action.responseJson.params.item != null && action.responseJson.params.item[classModel.field]) {
							inputFields[prefForms.PM_TESTCASE_FORM[i].name] = action.responseJson.params.item[classModel.field];
						} else {
							let result = "";
							if (prefForms.PM_TESTCASE_FORM[i].value != null && prefForms.PM_TESTCASE_FORM[i].value != ""){
								let formValue = JSON.parse(prefForms.PM_TESTCASE_FORM[i].value);
							...
							inputFields[prefForms.PM_TESTCASE_FORM[i].name] = result;
						}
					}
				}
		...
		case 'PM_TESTCASE_INPUT_CHANGE': {
			return reducerUtils.updateInputChange(state,action);
		}
		case 'PM_TESTCASE_CLEAR_FIELD': {
			return reducerUtils.updateClearField(state,action);
		}
		case 'PM_TESTCASE_LISTLIMIT': {
