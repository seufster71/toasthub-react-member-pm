  ...
  return function(dispatch) {
    ...
    return callService(params).then( (responseJson) => {
    	if (responseJson != null && responseJson.protocalError == null){
    		dispatch({ type: "PM_BACKLOG_INIT", responseJson });
		} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_BACKLOG_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function searchChange({field,value}) {
	return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"PM_BACKLOG_SEARCH_CHANGE",params});
	 };
}
...
export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_BACKLOG_SET_ERRORS",errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"PM_BACKLOG_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"PM_BACKLOG_CLOSE_DELETE_MODAL"});
	 };
}

export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"PM_BACKLOG_CANCEL"});
		dispatch(list({state}));
	 };
}