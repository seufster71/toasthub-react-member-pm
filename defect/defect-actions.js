...
export function init({parent,parentType}) {
  return function(dispatch) {
    ...
    requestParams.service = "PM_DEFECT_SVC";
    requestParams.prefTextKeys = new Array("PM_DEFECT_PAGE");
    requestParams.prefLabelKeys = new Array("PM_DEFECT_PAGE");
    if (parent != null) {
		...
		dispatch({type:"PM_DEFECT_ADD_PARENT", parent, parentType});
	} else {
		dispatch({type:"PM_DEFECT_CLEAR_PARENT"});
	}
    let params = {};
    ...
    return callService(params).then( (responseJson) => {
    	if (responseJson != null && responseJson.protocalError == null && responseJson.status != null && (responseJson.status == "ACTIONFAILED" || responseJson.status == "EXECUTIONFAILED")) {
    		...
    		dispatch({type: "PM_DEFECT_INIT", responseJson });
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		dispatch({type:"PM_DEFECT_PREF_CHANGE", userPrefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/member/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null && responseJson.status != null && (responseJson.status == "ACTIONFAILED" || responseJson.status == "EXECUTIONFAILED")) {
	    		dispatch({type:'SHOW_STATUS',error:responseJson.errors});
	    		dispatch({type: "PM_DEFECT_LIST", responseJson });
	    	} else if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "PM_DEFECT_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"PM_DEFECT_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"PM_DEFECT_SEARCH",searchCriteria});
...
export function searchChange({field,value}) {
	return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"PM_DEFECT_SEARCH_CHANGE",params});
	 };
}
...
export function modifyItem({id,appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "PM_DEFECT_SVC";
	    requestParams.prefFormKeys = new Array("PM_DEFECT_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'PM_DEFECT_ITEM',responseJson,appPrefs});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"PM_DEFECT_INPUT_CHANGE",params});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_DEFECT_ORDERBY",orderCriteria});
...
export function clearItem() {
	return function(dispatch) {
		dispatch({ type:"PM_DEFECT_CLEAR_ITEM"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"PM_DEFECT_CLEAR_FIELD",params});
	};
}

export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_DEFECT_SET_ERRORS",errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"PM_DEFECT_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"PM_DEFECT_CLOSE_DELETE_MODAL"});
	 };
}

export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"PM_DEFECT_CANCEL"});
		dispatch(list({state}));
	 };
}