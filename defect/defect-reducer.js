	...
	switch(action.type) {
		...
		case 'PM_DEFECT_ITEM': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				...
				for (let i = 0; i < prefForms.PM_DEFECT_FORM.length; i++) {
					if (prefForms.PM_DEFECT_FORM[i].group === "FORM1") {
						let classModel = JSON.parse(prefForms.PM_DEFECT_FORM[i].classModel);
						if (action.responseJson.params.item != null && action.responseJson.params.item[classModel.field] != null) {
							inputFields[prefForms.PM_DEFECT_FORM[i].name] = action.responseJson.params.item[classModel.field];
						} else {
							let result = "";
		...
		case 'PM_DEFECT_INPUT_CHANGE': {
			return reducerUtils.updateInputChange(state,action);
		}
		case 'PM_DEFECT_CLEAR_FIELD': {
			return reducerUtils.updateClearField(state,action);
		}
		case 'PM_DEFECT_LISTLIMIT': {
			return reducerUtils.updateListLimit(state,action);
		}
		case 'PM_DEFECT_SEARCH': { 
			return reducerUtils.updateSearch(state,action);
		}
		case 'PM_DEFECT_SEARCH_CHANGE': { 
